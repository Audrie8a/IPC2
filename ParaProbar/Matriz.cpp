...
void Matriz::getDot(FILE* file, Nodo* Cabeza)
{
	...
	Nodo* auxNodo = Cabeza->Abajo->Siguiente;
	Nodo* auxNodoaux= auxNodo;
	string enlacesUsr = "";
	filas++;
	do {
		auxNodo = auxNodoaux;
		do{
			...
			if (auxNodo->Anterior !=nullptr && auxNodo->Siguiente != nullptr) {				
				...
				if (auxNodo->Siguiente != nullptr) {
					enlacesUsr = auxNodo->Nombre + "->" + auxNodo->Siguiente->Nombre + "\n";
...
void Matriz::MenuUsuario(){
	...
	string Respuesta2;		//SOLO LA USO PARA PRUEBAS
	...
	do {
		...
		cout << "Opción 6: Devolver Activos" << endl;
		cout << "Opción 7: Cerrar Sesion" << endl;
		cin >> opcion;
		...
		case 5:
			...
			RaizArbol= arbol->getRaiz();
			arbol->MisActivos(RaizArbol);
			std::system("pause");
			break;
		case 6:
			cout << "%%%%%%%%%%%%%%%%%%% ACTIVOS RENTADOS %%%%%%%%%%%%%%%%%%%%%%" << endl;

			//Falta listar Mis activos
			cout << "¿Desea Devolver una Acitvo?		s/n" << endl;
			cin >> Respuesta2;
			if (Respuesta2 == "s" || Respuesta2 == "S") {
				cout << "------------Devolviendo Activo----------" << endl;
				cout << "Ingrese el ID del activo a devolver:" << endl;
				cin >> idDevuelto;
				BuscarArbol(idDevuelto, 2);
...
void Matriz::BuscarArbol(string ID, int numero)
{
	...
	if (Aux->Siguiente->Abajo != nullptr) {
		...
		do {
			...
			do {
				...
				if (constructorAVL != NULL) {
					NodoAVL = Aux2->getAVL()->getRaiz();
					constructorAVL->CambiarEstadoNodo(ID, NodoAVL, numero);
